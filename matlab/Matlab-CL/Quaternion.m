...
% written: q = s <vx, vy, vz>, it's a Hamilton type quaternion type.
%
...
% This code was stolen from https://github.com/petercorke/robotics-toolbox-matlab
% and inv function was modified by ShiQin
% dot function was modified by ShiQin 2018/03/28
...
classdef Quaternion
    ...
    methods
        ...
        function qc = conj(q)
        %Quaternion.inv Conjugate of a quaternion
        %
        % QI = Q.conj() is a quaternion object representing the conjugate of Q.

            qc = Quaternion([q.s -q.v]);
        end
        
        function qi = inv(q)
        % TODO: inverse is not conjugate
...
%             qd = Quaternion([-0.5*q.v*omega; 0.5*E*omega]);
        q = q.unit; % added by Qin Shi 2018.12.18
        omega = omega(:);
        qd = 1/2 * q * Quaternion(omega);
        end
    end % methods
end % classdef
